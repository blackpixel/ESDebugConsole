...
- (void)showFromView:(UIView *)view
{
	...
	if (ISPAD)
	{
		...
		[self.popoverController presentPopoverFromRect:CGRectMake(0, 0, 10, 10) inView:view permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
	}
	else if (ISPHONE)
	{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	{
		tvc.applicationLogs = self.allApplicationLogs[applicationIdentifier];
...
- (void)email:(UIBarButtonItem *)sender
{
	...
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^(void) {
		NSArray *logs = [ESDebugConsole getConsole][kESDebugConsoleAllLogsKey];
		dispatch_async(dispatch_get_main_queue(), ^(void) {
			[[ESDebugConsole sharedDebugConsole] performSelector:@selector(setMessage:) withObject:[logs esdebug_formattedConsoleString]];
...
- (void)refreshTimer:(NSTimer *)timer
{
	...
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
		NSArray *newLogs = [ESDebugConsole getConsole][self.applicationIdentifier];
		dispatch_async(dispatch_get_main_queue(), ^(void) {
...
{
	[[ESDebugConsole sharedDebugConsole] performSelector:@selector(setMessage:) withObject:[self.applicationLogs esdebug_formattedConsoleString]];
	[[ESDebugConsole sharedDebugConsole] performSelector:@selector(sendConsoleAsEmail)];
}
